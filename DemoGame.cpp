...
namespace WarGame {
		...
		uint DemoGame::play() {
			...
			board.move(2, {7,3}, Board::MoveDIR::Down);    // FootSoldier of player 2 moves forward and attacks from {7,1} to {6,1}.
			...
			// std::cout<<board[{7,1}]->getLife();

			board.move(2, {7,1}, Board::MoveDIR::Left);    // FootCommander of player 2 moves left from {7,3} to {7,2}, and all soldiers of player 2 attack.
			if (!board.has_soldiers(1)) return 2;
